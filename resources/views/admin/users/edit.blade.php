...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="course_id">{{ trans('cruds.user.fields.course') }}</label>
                <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id">
                    @foreach($courses as $id => $course)
                        <option value="{{ $id }}" {{ ($user->course ? $user->course->id : old('course_id')) == $id ? 'selected' : '' }}>{{ $course }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="package_id">{{ trans('cruds.user.fields.package') }}</label>
                <select class="form-control select2 {{ $errors->has('package') ? 'is-invalid' : '' }}" name="package_id" id="package_id">
                    @foreach($packages as $id => $package)
                        <option value="{{ $id }}" {{ ($user->package ? $user->package->id : old('package_id')) == $id ? 'selected' : '' }}>{{ $package }}</option>
                    @endforeach
                </select>
