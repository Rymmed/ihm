    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.packages.update", [$package->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="text" name="duration" id="duration" value="{{ old('duration', $package->duration) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('courses') ? 'is-invalid' : '' }}" name="courses[]" id="courses" multiple required>
                    @foreach($courses as $id => $courses)
                        <option value="{{ $id }}" {{ (in_array($id, old('courses', [])) || $package->courses->contains($id)) ? 'selected' : '' }}>{{ $courses }}</option>
                    @endforeach
                </select>
